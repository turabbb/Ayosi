...
export const ProductCard = ({ product }: { product: Product }) => {
  ...
  return (
    <Card 
      className="group overflow-hidden hover:shadow-2xl transition-all duration-500 border-0 shadow-sm hover:border hover:border-primary/20 transform hover:-translate-y-1"
    ...
    >
      <div className="relative cursor-pointer overflow-hidden" onClick={() => navigate(`/product/${product.id}`)}>
        ...
        {/* Featured badge */}
        {product.isFeatured && (
          <div className="absolute top-3 left-3 bg-primary text-primary-foreground text-xs font-semibold px-2 py-1 rounded-full shadow-lg">
            Featured
          </div>
        )}
      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-2">
          <div className="cursor-pointer" onClick={() => navigate(`/product/${product.id}`)}>
            <h3 className="text-sm font-medium group-hover:text-primary transition-colors duration-200 line-clamp-2 leading-tight">
              {product.title}
            </h3>
            <p className="text-sm font-bold text-primary mt-1">Rs. {Math.round(product.price)}</p>
          </div>
          <Button 
            size="icon" 
            variant="ghost" 
